...
class Ambulance(pygame.sprite.Sprite):
    def __init__(self, direction):
        pygame.sprite.Sprite.__init__(self)
        self.speed = speeds['car'] * 2  # Ambulance moves faster
        self.direction = direction
        self.x = x[direction][1]  # Lane 1 for ambulance
        self.y = y[direction][1]
        self.image = pygame.image.load('C:\Users\reesh\Downloads\Traffic-Intersection-Simulation-with-Turns-main\ambulance.png')  # Use an ambulance image
        self.crossed = False
        simulation.add(self)
    ...
    def move(self):
        ...
        if self.x < 0 or self.x > screenWidth or self.y < 0 or self.y > screenHeight:
            self.crossed = True  # Ambulance left the intersection

def generateAmbulances():
    while True:
        ambulance_direction = random.choice(list(directionNumbers.values()))
        ambulance = Ambulance(ambulance_direction)

        # Stop other vehicles and set all signals to red
        for i in range(noOfSignals):
            signals[i].red = max(signals[i].red, 999)  # Ensure signals stay red

        while not ambulance.crossed:
            time.sleep(0.1)

        # Restore signal timings
        for i in range(noOfSignals):
            if randomGreenSignalTimer:
        ...
        simulation.remove(ambulance)  # Remove the ambulance from simulation
        time.sleep(30)  # New ambulance every 30 seconds

# Add a thread to generate ambulances
thread3 = threading.Thread(name="generateAmbulances", target=generateAmbulances, args=())
thread3.daemon = True
thread3.start()
